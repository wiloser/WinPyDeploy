...
class InstallerWorker:
    ...
    def stop(self) -> None:
        self._stop.set(); self._runner.terminate()

    def cancel_app(self, app_id: str) -> None:
        self._cancelled.add(app_id); self._current == app_id and self._runner.terminate()
    ...
    def install(self, apps: list[AppSpec]) -> None:
        ...
        for app in apps:
            ...
            if self._stop.is_set():
                ...
                continue
            if app.app_id in self._cancelled:
                self._q.put(InstallEvent("skipped", app.app_id, "cancelled"))
                continue
            ...
            for cmd in app.install_commands:
                ...
                if code != 0:
                    if app.app_id in self._cancelled:
                        self._q.put(InstallEvent("skipped", app.app_id, "cancelled")); failed = True; break
                    self._q.put(InstallEvent("failed", app.app_id, f"安装失败（exit={code}），停止后续任务")); failed = True; self._stop.set(); break
